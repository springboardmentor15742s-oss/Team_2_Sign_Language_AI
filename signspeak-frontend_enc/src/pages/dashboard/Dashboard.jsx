...
export default function Dashboard() {
  const { user } = useAuth();
  const [data, setData] = useState({ learning: null, stats: null, courses: [], enrolled: [], notifications: [] });
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      setData({
        learning: value(1), stats: value(2),
        courses: value(3) || [], enrolled: value(4) || [], notifications: value(5) || [],
      });
    });
    return () => { active = false; };
  }, []);
  ...
  const weekly = useMemo(() => {
    const activity = data.stats?.weekly_activity;
    if (Array.isArray(activity) && activity.length) {
      return activity.map((d) => ({ day: d.day, minutes: d.count }));
    }
    return ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => ({ day, minutes: 0 }));
  }, [data.stats]);

  const maxMinutes = Math.max(4, ...weekly.map((d) => d.minutes));
  const accuracy = Math.round(Number(data.stats?.gesture_accuracy || 0));
  ...
  const totalXp = data.stats?.total_xp ?? user?.xp_points ?? 0;
  const currentStreak = data.stats?.current_streak ?? user?.current_streak ?? 0;
  const achievements = data.stats?.achievements_count ?? 0;
    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-slate-800 bg-[#111827] px-6 py-7 lg:px-9 lg:py-8 shadow-card">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
          <div>
            ...
            <Link to="/courses" className="mt-5 inline-flex items-center justify-center gap-2 rounded-xl bg-[#16c8c4] hover:bg-[#20d8d3] text-slate-950 px-6 py-3 text-sm font-bold shadow-[0_0_30px_rgba(20,201,197,0.18)] transition-colors">
          ...
          <div className="flex flex-wrap gap-3 shrink-0">
            {[
              [Flame, currentStreak, 'Day Streak', 'text-amber-400'],
              [Zap, totalXp, 'Total XP', 'text-[#20d8d3]'],
              [Target, `${accuracy}%`, 'Gesture Accuracy', 'text-cyan-400'],
              [Trophy, achievements, 'Achievements', 'text-violet-400'],
              ...
              <motion.div
                ...
                className="flex items-center gap-2.5 rounded-2xl bg-white/5 border border-white/10 px-4 py-3 min-w-[128px]"
              >
      ...
      <div className="grid xl:grid-cols-[1.6fr_0.8fr] gap-6">
        <Card padding="large">
          <div className="flex items-start justify-between mb-6">
            <div><div className="flex items-center gap-2 text-sm font-bold text-white"><BarChart3 size={17} className="text-[#20d8d3]" /> Weekly Activity</div><p className="mt-1 text-xs text-slate-500">Sign attempts per day this week</p></div>
      ...
      <Card padding="large">
        <div className="flex items-center justify-between mb-5">
          <div><p className="text-xs font-semibold uppercase tracking-wider text-[#20d8d3]">Activity</p><h2 className="mt-1 text-xl font-bold text-white">Recent Assessments & Practice</h2></div>
          <Link to="/assessments" className="text-xs font-semibold text-[#20d8d3] flex items-center gap-1">View all <ChevronRight size={14} /></Link>
        </div>
        {data.stats?.recent_activity?.length ? (
          <div className="space-y-2">
            {data.stats.recent_activity.map((item, i) => (
              <div key={i} className="flex items-center justify-between gap-3 rounded-xl border border-slate-800 p-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 ${item.type === 'assessment' ? 'bg-violet-500/10 text-violet-400' : 'bg-cyan-500/10 text-cyan-400'}`}>
                    {item.type === 'assessment' ? <Award size={16} /> : <Hand size={16} />}
                  </div>
                  <div className="min-w-0">
                    {item.type === 'assessment' ? (
                      <p className="text-sm font-semibold text-slate-200 truncate">{item.title} &middot; {item.correct}/{item.total} correct ({item.score}%)</p>
                    ) : (
                      <p className="text-sm font-semibold text-slate-200 truncate">
                        Sign {item.sign} &middot; {item.correct ? 'Correct' : 'Incorrect'} &middot; {Math.round((item.confidence || 0) * 100)}% confidence
                      </p>
                    )}
                ...
                {item.type === 'practice' && (
                  <CheckCircle2 size={16} className={item.correct ? 'text-emerald-400 shrink-0' : 'text-slate-600 shrink-0'} />
                )}
              </div>
            ))}
          </div>
        ) : (
          <div className="rounded-xl border border-dashed border-slate-800 p-7 text-center">
            <Hand className="mx-auto text-slate-600" size={28} />
            <p className="mt-3 text-sm text-slate-400">No practice or assessment activity yet.</p>
            <Link to="/practice" className="mt-3 inline-flex text-xs font-semibold text-[#20d8d3]">Start practicing</Link>
          </div>
        )}
      </Card>
