...
@router.post("/token", response_model=AccessTokenResponse, include_in_schema=True)
def oauth2_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    """OAuth2 password-flow compatible token endpoint for Swagger/clients."""
    user = authenticate_user(db, form_data.username, form_data.password)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid email or password", headers={"WWW-Authenticate": "Bearer"})
    if not user.is_active:
        raise HTTPException(status_code=403, detail="Account is deactivated")
    return AccessTokenResponse(access_token=create_access_token(str(user.id), {"role": user.role.value}))
...
def register(user_in: UserCreate, db: Session = Depends(get_db)):
    ...
    # Public registration can only create learner accounts. Privileged roles
    # are assigned by an administrator through the protected admin API.
    if user_in.role.value != "student":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Public registration is restricted to learner accounts",
        )
...
def login(credentials: LoginRequest, db: Session = Depends(get_db)):
    ...
    log_activity(db, user.id, "LOGIN", meta=f"role={user.role.value}")
    db.commit()
    db.refresh(user)
