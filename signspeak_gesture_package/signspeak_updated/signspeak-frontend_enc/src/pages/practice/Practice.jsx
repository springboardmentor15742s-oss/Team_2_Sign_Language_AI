...
export default function Practice() {
  ...
  const handleFrame = (detection) => {
    setLatestDetection(detection);
    if (detection?.gesture && detection.gesture !== 'No hand') {
      setConfidence(Math.round((detection.confidence || 0) * 100));
      setDetections((items) => [
        ...items.slice(-99),
        { gesture: detection.gesture, confidence: detection.confidence || 0, timestamp: detection.timestamp },
      ]);
    }
  };

  const analyzeCurrentSign = () => {
    const score = Math.round((latestDetection?.confidence || 0) * 100);
    const detected = latestDetection?.gesture || 'No hand';
    const matches = detected.toLowerCase() === currentSign.name.replace('Letter ', '').toLowerCase();

    setAttempts((value) => value + 1);
    if (matches) setSuccessfulAttempts((value) => value + 1);
    setConfidence(score);
    setFeedback({
      type: matches && score >= 70 ? 'success' : score >= 50 ? 'warning' : 'error',
      title: matches && score >= 70 ? 'Correct sign' : matches ? 'Good shape — improve confidence' : `Detected ${detected}`,
      message: matches && score >= 70
        ? `Your ${currentSign.name} matched the target gesture with ${score}% confidence.`
        : matches
          ? `The target ${currentSign.name} was detected, but keep your hand steadier and better centered.`
          : `The engine detected ${detected}. Recheck the target hand shape for ${currentSign.name} and try again.`,
    });
  };
      ...
      <div className="grid xl:grid-cols-[1.45fr_0.8fr] gap-6 items-start">
        <div className="space-y-5">
          <Card padding="none" className="overflow-hidden">
            ...
            <div className="p-4"><CameraPanel targetGesture={currentSign.name.replace('Letter ', '')} onFrame={handleFrame} /></div>
          </Card>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <Card>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <div className="rounded-xl bg-slate-50 p-2"><p className="text-[10px] uppercase tracking-wide text-slate-400">Detected</p><p className="mt-1 text-sm font-bold text-slate-800">{latestDetection?.gesture || '—'}</p></div>
                <div className="rounded-xl bg-slate-50 p-2"><p className="text-[10px] uppercase tracking-wide text-slate-400">Attempts</p><p className="mt-1 text-sm font-bold text-slate-800">{attempts}</p></div>
                <div className="rounded-xl bg-slate-50 p-2"><p className="text-[10px] uppercase tracking-wide text-slate-400">Correct</p><p className="mt-1 text-sm font-bold text-emerald-600">{successfulAttempts}</p></div>
              </div>
            </Card>
        ...
        <div className="space-y-5">
          <Card padding="large">
            ...
            <div className="mt-5 space-y-2">
              <Button className="w-full" onClick={analyzeCurrentSign}><Hand size={16} /> Analyze my sign</Button>
          ...
          <Card>
            ...
            <div className="space-y-2">
              {PRACTICE_SIGNS.map((sign, i) => (
                <button key={sign.id} onClick={() => { setCurrentIndex(i); setFeedback(null); setConfidence(0); setLatestDetection(null); }} className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all ${i === currentIndex ? 'bg-primary-50 border border-primary-100' : 'hover:bg-slate-50 border border-transparent'}`}>
